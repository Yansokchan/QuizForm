  ...
  return text.split('').map((char, i) => {
    ...
    return (
      <span
        key={`${i}-${char}`}
        data-label={char}
        className={isSpace ? 'char-gap' : undefined}
        style={{ ['--i']: i + 1 }}
      >
        {char}
      </span>
        ...
        <div className="wrap">
          ...
          <div className="content">
            <div className="label-slot">
              <span className="char state-1">{charSpans(primaryText)}</span>
              <span className="char state-2">{charSpans(secondaryText)}</span>
            </div>
            <div className="icon">
              <div />
            </div>
...
const StyledWrapper = styled.div`
  ...
  @media (max-width: 768px) {
    .button {
      width: ${(p) => Math.min(p.$btnWidth ?? 220, 268)}px;
      height: 64px;
      font-size: 17px;
      --radius: 14px;
    }
    ...
    .splash {
      transform: translate(-17%, -31%) scale(0.88);
    }

    .bg::before {
      box-shadow:
        -5px 4px 0 0 rgb(115 75 155 / 40%),
        -10px 8px 0 0 rgb(115 75 155 / 28%),
        -16px 14px 3px 0 rgb(115 75 155 / 18%),
        -22px 18px 6px 0 rgb(115 75 155 / 10%);
    }
  ...
  @media (max-width: 480px) {
    .button {
      width: ${(p) => Math.min(p.$btnWidth ?? 220, 236)}px;
      height: 56px;
      font-size: 15px;
      --radius: 12px;
    ...
    .splash {
      transform: translate(-17%, -31%) scale(0.8);
    }
  }
`;
